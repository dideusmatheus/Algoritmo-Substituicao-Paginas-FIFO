...
int fifo(int8_t** page_table, int num_pages, int prev_page,
         int fifo_frm, int num_frames, int clock) {
            //percorrendo toda paginas
            int i;
            //vemos qua o id da primeira page acessada pela fila
            int valor=page_table[prev_page][PT_FRAMEID];
            //verificamos entre as paginas mapeadas qual é a que esta alocada no menor frame id, esta pagina é a que foi  alocada primeiramente
             for(i=0;i<num_pages;i++){
                 
                 if((page_table[i][PT_FRAMEID]<valor || page_table[i][PT_FRAMEID] == page_table[prev_page][PT_FRAMEID]+1 ) && page_table[i][PT_MAPPED]!=0  ){
